...
const GameCard = ({game, team}) => (
    <div className="w-full px-4 grid grid-cols-6 bg-gray rounded">
        ...
        <Score  game={game} teamId={team == "home" ? game.homeTeamId : game.awayTeamId}></Score>
        </div>
        <Link 
            key={game.id+game.homeTeamId} 
            href={{pathname:`/game/${game.id}/${team == "home" ? game.homeTeamId : game.awayTeamId}`}} 
    ...
    return (
        <div className='grow justify-between space-y-4'>
            {Object.keys(games_by_date).reverse().map(date => <div key={date}> <h1 className='grid justify-items-center'>{date}</h1>{games_by_date[date].map(gameId => {
                ...
                return (
                    <div key={gameId} className='grid grid-rows-2  w-full m-2 bg-gray-900 rounded'>
                        <GameCard className='row-start-1 row-span-1' game={game} team="home" /> 
                        <GameCard className='row-start-2 row-span-1' game={game} team="away" />
                        <div className="w-full px-4 grid grid-cols-6 bg-gray rounded">
                            <Link
